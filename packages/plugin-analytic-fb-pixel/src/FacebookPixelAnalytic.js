...
export default class FacebookPixelAnalytic extends AbstractAnalytic {
	...
	createGlobalDefinition(window) {
		...
		const fbAnalytic = window[FB_ROOT_VARIABLE] = function () {
			fbAnalytic.callMethod ? fbAnalytic.callMethod.apply(fbAnalytic, arguments) : fbAnalytic.queue.push(arguments)
		};
	...
	_configuration() {
		...
		if (!clientWindow[FB_ROOT_VARIABLE] || typeof clientWindow[FB_ROOT_VARIABLE] !== 'function') {
			return;
		}

import AbstractRestClientCache from './AbstractRestClientCache';
import ImaSharedCache from '@ima/plugin-shared-cache';

const REST_CLIENT_SHARED_CACHE_NAME = 'IMA.js REST API client';
const SharedCacheInstance = ImaSharedCache.getCache(REST_CLIENT_SHARED_CACHE_NAME);
...
export default class SharedCache extends AbstractRestClientCache {
	...
	createCacheEntry(response) {
		...
		SharedCacheInstance.set(cacheKey, this.composeResponseCacheData(response), ttl);
	}
